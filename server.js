...
async function getMatches(forceRefresh) {
  ...
  const freeSources = new Set(['espn-with-fallbacks', 'tff-thesportsdb', 'tff-sportscore', 'demo']);
...
async function buildCoupon(surprise=false,league='all'){
  ...
  const candidates=rows.flatMap(({match,analysis})=>{if(!analysis?.recommendation?.primary)return[];const p=analysis.recommendation.primary,dataConfidence=Number(analysis.scores?.confidence||0),h2h=Math.min(Number(analysis.h2h?.played||0),5),confidence=Math.round(p.confidence*.72+dataConfidence*.23+h2h);if(p.confidence<55||dataConfidence<55||confidence<58)return[];return[{matchId:match.id,home:match.home,away:match.away,date:match.date,selection:p.market,confidence:Math.min(confidence,89),reason:p.reason,lineupConfirmed:Boolean(analysis.lineup?.confirmed),sources:(analysis.sources||[]).map(x=>x.name)}]}).sort((a,b)=>b.confidence-a.confidence).slice(0,5);
