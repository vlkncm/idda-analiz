...
function recommendation(r){const picks=(r.picks||[]).map(p=>`<li><strong>${r.primary&&p.market===r.primary.market?'EN GÜÇLÜ: ':'ALTERNATİF: '}${p.market}</strong> · olasılık %${p.confidence} · risk ${p.risk}<small>${p.reason}</small></li>`).join('');return `<section class="decision ${r.action==='VERİ YETERSİZ'?'decision-pass':''}"><span>MODEL DEĞERLENDİRMESİ</span><h3>${r.action}</h3><p><b>Risk:</b> ${r.risk} · <b>Üst sınır:</b> ${r.stake}</p>${picks?`<ol>${picks}</ol>`:'<p>Bu maç için yeterli iç/dış saha geçmişi bulunamadı.</p>'}${r.avoid?.length?`<p><b>Uzak dur:</b> ${r.avoid.join(' · ')}</p>`:''}${r.comboWarning?`<p class="decision-warning">${r.comboWarning}</p>`:''}${r.warnings?.length?`<ul class="decision-warnings">${r.warnings.map(x=>`<li>${x}</li>`).join('')}</ul>`:''}</section>`}
