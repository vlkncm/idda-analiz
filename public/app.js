...
function formTable(name,data={}){return `<h3>${esc(name)}</h3><table><tr><td>Örnek (son 5 / son 10)</td><td>${valueOrDash(data.windows?.last5?.played)} / ${valueOrDash(data.windows?.last10?.played??data.played)}</td></tr><tr><td>Maç / G-B-M</td><td>${valueOrDash(data.played)} / ${valueOrDash(data.wins)}-${valueOrDash(data.draws)}-${valueOrDash(data.losses)}</td></tr><tr><td>Galibiyet / beraberlik</td><td>${valueOrDash(rate(data.winRate),'%')} / ${valueOrDash(rate(data.drawRate),'%')}</td></tr><tr><td>Puan/maç</td><td>${valueOrDash(data.pointsPerMatch)}</td></tr><tr><td>Attığı / yediği gol</td><td>${valueOrDash(data.goalsForAvg)} / ${valueOrDash(data.goalsAgainstAvg)}</td></tr><tr><td>2,5 Üst / KG Var</td><td>${valueOrDash(rate(data.over25),'%')} / ${valueOrDash(rate(data.btts),'%')}</td></tr><tr><td>Son 5 form</td><td>${esc(data.windows?.last5?.form||'—')}</td></tr><tr><td>Son 10 form</td><td>${esc(data.form||'—')}</td></tr></table>`;}
