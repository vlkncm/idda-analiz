...
async function getModern(url) { const r=await fetch(url,{headers:{'user-agent':UA,'accept-language':'tr-TR,tr;q=0.9,en;q=0.7'}});if(!r.ok)throw new Error(`Kaynak ${r.status}`);return r.text(); }
