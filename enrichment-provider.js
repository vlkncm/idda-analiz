...
function getLegacy(url) { return new Promise((resolve,reject)=>https.get(url,{signal:AbortSignal.timeout(8000),headers:{'User-Agent':UA}},r=>{if(r.statusCode!==200){r.resume();return reject(new Error(`TFF ${r.statusCode}`));}const chunks=[];r.on('error',reject);r.on('data',c=>chunks.push(c));r.on('end',()=>resolve(new TextDecoder('windows-1254').decode(Buffer.concat(chunks))));}).on('error',reject)); }
const pageInFlight = new Map();
async function getModern(url) {
  const hit = pageCache.get(url);
  if (hit && Date.now() - hit.time < 30 * 60000) return hit.value;
  if (pageInFlight.has(url)) return pageInFlight.get(url);
  const operation = fetch(url, { signal: AbortSignal.timeout(8000), headers: { 'user-agent': UA, 'accept-language': 'tr-TR,tr;q=0.9,en;q=0.7' } })
    .then(async response => { if (!response.ok) throw new Error(`Kaynak ${response.status}`); return response.text(); })
    .then(value => { pageCache.set(url, { time: Date.now(), value }); return value; });
  pageInFlight.set(url, operation);
  try { return await operation; } finally { pageInFlight.delete(url); }
}
...
async function enrichMatch(match) {
  ...
  const matchId=match.provider === 'tff' ? String(match.providerMatchId || '').match(/^\d+$/)?.[0] : null;
  const tasks=[];
