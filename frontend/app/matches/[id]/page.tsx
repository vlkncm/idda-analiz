...
import GlobalBetting from "../../../components/GlobalBetting";
import { getGlobalBetting, getMatch, getMatchPrediction } from "../../../lib/api";

const pct = (value: unknown) => `${(Number(value) * 100).toFixed(1)}%`;

export default async function MatchDetail({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ tab?: string }> }) {
  const { id } = await params;
  const { tab = "analysis" } = await searchParams;
  const [match, prediction, globalBetting] = await Promise.all([getMatch(id), getMatchPrediction(id), tab === "betting" ? getGlobalBetting(id) : Promise.resolve(null)]);
  if (!match) notFound();
  const features = (prediction?.feature_snapshot ?? {}) as Record<string, number>;
  const reasons = (prediction?.explanation ?? []) as string[];
  return <main>
    <section className="hero compact"><p>MAÇ ANALİZİ</p><h1>{String(match.home_team)}<br /><em>{String(match.away_team)}</em></h1><span>{new Intl.DateTimeFormat("tr-TR", { dateStyle: "full", timeStyle: "short", timeZone: "Europe/Istanbul" }).format(new Date(String(match.kickoff_at)))} (Türkiye)</span></section>
    <nav className="analysis-tabs"><Link className={tab === "analysis" ? "active" : ""} href={`/matches/${id}`}>Model Analizi</Link><Link className={tab === "betting" ? "active" : ""} href={`/matches/${id}?tab=betting`}>Global Bahis</Link></nav>
    {tab === "betting" ? <GlobalBetting data={globalBetting} /> : prediction ? <>
      <section className="detail-markets"><div><small>1</small><strong>{pct(prediction.home_probability)}</strong></div><div><small>X</small><strong>{pct(prediction.draw_probability)}</strong></div><div><small>2</small><strong>{pct(prediction.away_probability)}</strong></div><div><small>2.5 ÜST</small><strong>{pct(prediction.over25_probability)}</strong></div></section>
    ...
    </> : <div className="empty">Bu maç için henüz production model tahmini oluşturulmadı.</div>}
  </main>;
}
